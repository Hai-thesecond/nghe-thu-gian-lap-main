...
const StudentDashboard = () => {
      ...
      checkAndRefreshSession().then(sessionOk => {
        if (sessionOk) {
          ...
          queryClient.refetchQueries({ 
            queryKey: ['student-answers', user?.id],
            type: 'active'
          });
          
          queryClient.refetchQueries({ 
            queryKey: ['student-questions', user?.id, activeTab],
            type: 'active'
          });
          ...
          queryClient.refetchQueries({ 
            queryKey: ['completed-assignments', user?.id],
            type: 'active'
          });
  ...
  }, [user?.id, activeTab, queryClient, checkAndRefreshSession, fetchCompletedAssignments]);
