...
const VocabularyPractice = () => {
  ...
  // Add a state to track if practice is in progress
  const [isPracticeStarted, setIsPracticeStarted] = useState(false);
  
  // Add a state to track if anti-cheating is enabled
  ...
  // Set practice started state when data is loaded and ready
  useEffect(() => {
    if (vocabularyList.length > 0 && !isCheckingPreviousStep && !isOnCooldown) {
      setIsPracticeStarted(true);
      console.log('Practice started set to TRUE', { 
        vocabularyListLength: vocabularyList.length, 
        isCheckingPreviousStep, 
        isOnCooldown 
      });
    } else {
      console.log('Practice NOT started yet', { 
        vocabularyListLength: vocabularyList.length, 
        isCheckingPreviousStep, 
        isOnCooldown 
      });
    }
  }, [vocabularyList, isCheckingPreviousStep, isOnCooldown]);

  // New function to handle mouse leave detection - removing handleCheatingAttempt from dependencies
  const handleMouseLeave = useCallback((event: MouseEvent) => {
    console.log('Mouse leave detected', { isPracticeStarted, event: event.type });
  ...
  // New function to handle visibility change (tab switching) - removing handleCheatingAttempt from dependencies
  const handleVisibilityChange = useCallback(() => {
    console.log('Visibility change detected', { 
      visibilityState: document.visibilityState, 
      isPracticeStarted 
    });
    if (document.visibilityState === 'hidden' && isPracticeStarted) {
    ...
    if (user?.id && questionId) {
      ...
      supabase
        ...
        .then(({ data: profileData, error: profileError }) => {
          ...
          supabase
            .from('student_answers')
            .select('attempt_count, cheating_attempts, id')
            .eq('student_id', user.id)
            .eq('question_id', questionId)
            ...
            .then(({ data, error }) => {
              ...
              // Kiểm tra xem có bản ghi hiện tại không
              if (data && data.length > 0 && data[0].id) {
                const recordId = data[0].id;
                const updatedAttempts = (data[0].cheating_attempts || 0) + 1;
                ...
                supabase
                  ...
                  .then(({ error: insertError }) => {
                    if (insertError) {
                      ...
                      // Cập nhật state với số lần vi phạm mới nhất
                      setCheatingAttempts(1);
                    }
                  });
              }
              
              // Luôn cập nhật tổng số lần gian lận trong profiles
              supabase
                .from('profiles')
                .update({
  ...
  // Setup anti-cheating event listeners conditionally
  useEffect(() => {
    console.log('Setting up anti-cheating event listeners', { enableAntiCheating, isPracticeStarted });
    
    if (enableAntiCheating) {
      console.log('Anti-cheating enabled, attaching event listeners');
      
      // Use window instead of document for more reliable detection
      window.addEventListener('visibilitychange', handleVisibilityChange);
      document.addEventListener('mouseleave', handleMouseLeave);
      
      // Add mouseout event for better detection
      const handleMouseOut = (event: MouseEvent) => {
        // Only trigger if the mouse leaves the document
        if (!event.relatedTarget && isPracticeStarted) {
          console.log('Mouse out event detected (leaving document)');
          handleCheatingAttempt('Di chuyển chuột ra khỏi cửa sổ (mouseout)');
        }
      };
      
      // Handler for keyboard events (F12 or Ctrl+Shift+I)
      const handleKeyDown = (event: KeyboardEvent) => {
        if ((event.key === 'F12' || (event.ctrlKey && event.shiftKey && event.key === 'I')) && isPracticeStarted) {
          console.log('F12 or Ctrl+Shift+I detected');
          handleCheatingAttempt('Mở Developer Tools (F12)');
          event.preventDefault();
        }
      };
      
      // Handler for window blur
      const handleWindowBlur = () => {
        if (isPracticeStarted) {
          console.log('Window blur detected');
          handleCheatingAttempt('Chuyển khỏi cửa sổ trình duyệt');
        }
      };
      ...
      // Handle when mouse moves too quickly
      const handleMouseMove = (event: MouseEvent) => {
        if (isPracticeStarted) {
          const now = Date.now();
          // Store current mouse position and time
          const mouseState = {
            x: event.clientX,
            y: event.clientY,
            time: now
          };
          // Store in window object to persist between event calls
          const prevState = (window as any)._prevMouseState;
          if (prevState) {
            // Calculate speed (pixels per millisecond)
            const dx = mouseState.x - prevState.x;
            const dy = mouseState.y - prevState.y;
            const dt = mouseState.time - prevState.time;
            const speed = Math.sqrt(dx*dx + dy*dy) / Math.max(dt, 1);
            
            // If mouse moves extremely quickly to edge of screen (possible alt+tab)
            if (speed > 10 && (
                mouseState.x <= 5 || 
                mouseState.x >= window.innerWidth - 5 || 
                mouseState.y <= 5 || 
                mouseState.y >= window.innerHeight - 5
              )) {
              console.log('Suspicious mouse movement detected', { speed, dx, dy, dt });
              handleCheatingAttempt('Di chuyển chuột quá nhanh ra khỏi cửa sổ');
            }
          }
          (window as any)._prevMouseState = mouseState;
        }
      };
      ...
      return () => {
        console.log('Removing anti-cheating event listeners');
        ...
        // Clear temp state
        (window as any)._prevMouseState = null;
      };
    } else {
      console.log('Anti-cheating disabled, no event listeners attached');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [handleVisibilityChange, handleMouseLeave, enableAntiCheating, isPracticeStarted, resetInteractions]);
  ...
  useEffect(() => {
    if (questionId && user) {
      // Luôn bật tính năng anti-cheating mặc định
      setEnableAntiCheating(true);
      console.log('Anti-cheating: Bật mặc định để đảm bảo tính năng hoạt động');
      
      // Try to get question settings, but ignore the enable_anti_cheating value
      supabase
        .from('questions')
        ...
        .then(({ data, error }) => {
          if (!error && data) {
            console.log('Anti-cheating config từ database:', data.enable_anti_cheating);
            // Giữ nguyên giá trị true đã đặt ở trên, không sử dụng giá trị từ database
            // setEnableAntiCheating(data.enable_anti_cheating !== false);
          }
        });
    }
  }, [questionId, user]);
