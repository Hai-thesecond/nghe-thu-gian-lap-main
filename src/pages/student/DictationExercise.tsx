...
const DictationExercise = () => {
        ...
        .from('student_answers')
            ...
            .then(({ data: existingData, error: existingError }) => {
              ...
              if (existingData && existingData.length > 0) {
                ...
                // Tạo metadata object không có null values để tránh lỗi FOR loop
                const safeMetadata = {
                  terminated_reason: `Gian lận: ${reason}`,
                  terminated_at: new Date().toISOString(),
                  total_cheating_history: newTotalCheating || 0,
                  question_cheating_history: updatedAttempts || 0
                };
                
                // Kiểm tra dữ liệu trước khi tạo bản ghi
                console.log('Safe metadata object for cheating record:', safeMetadata);
                ...
                supabase
                  ...
                  .insert({
                    ...
                    cheating_attempts: updatedAttempts || 1,
                    is_completed: true,
                    completed_at: new Date().toISOString(),
                    ...
                    score: 0, // Add default values to prevent null
                    correct_count: 0,
                    incorrect_count: 0
                  })
                  .then(({ error: insertError }) => {
                    if (insertError) {
                      ...
                      // Nếu vẫn lỗi, thử tạo bản ghi đơn giản hơn
                      if (insertError.code === '22004') {
                        console.log('Thử tạo bản ghi đơn giản hơn do lỗi FOR loop');
                        supabase
                          .from('student_answers')
                          .insert({
                            student_id: user.id,
                          ...
                          .then(result => {
                            if (result.error) {
                              console.error('Vẫn lỗi khi tạo bản ghi đơn giản:', result.error);
                            } else {
                              console.log('Đã tạo bản ghi đơn giản thành công');
                            }
                          });
                      }
                    } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Cờ để kiểm soát việc sử dụng RPC hay chỉ dùng Direct API
      const useDirectApiOnly = true; // Đặt true để bỏ qua RPC và luôn dùng Direct API
      ...
      const metadata = {
        ...
        student_category: studentCategory.category,
        student_fill_percentage: getFillPercentage(studentCategory.category),
      ...
      // Thử RPC chỉ khi biến cờ cho phép 
      if (!useDirectApiOnly) {
        try {
          // Sử dụng save_new_student_attempt RPC để lưu kết quả
          console.log('Sử dụng save_new_student_attempt RPC để lưu kết quả...');
          
          const { data: rpcResult, error: rpcError } = await supabase.rpc('save_new_student_attempt', {
            p_student_id: user.id,
            p_question_id: questionId,
            p_score: scoreResults.totalScore,
            p_correct_count: scoreResults.correctAnswers,
            p_incorrect_count: scoreResults.incorrectAnswers,
            p_answers: safeAnswers,
            p_answers_text: answers_text,
            p_correct_answers_text: correct_answers_text,
            p_metadata: metadata,
      ...
      // Nếu RPC thất bại hoặc bị bỏ qua, sử dụng phương pháp dự phòng
      if (!submissionSuccess) {
        console.log('Thử sử dụng Direct API requests để lưu kết quả...');
        ...
        try {
          ...
          console.error('Lỗi khi sử dụng Direct API:', directApiError);
        }
        
        // Kiểm tra xem phương pháp dự phòng có thành công không
        if (!submissionSuccess) {
    ...
    try {
      ...
      const loadPromise = new Promise((resolve, reject) => {
        const onCanPlay = () => {
          ...
          console.log('Audio element state after successful load:', {
        ...
        const onError = (e: Event) => {
          console.error('Audio element error:', newAudio.error);
          
          // Log detailed error info
          console.error('Audio error details:', {
            code: newAudio.error ? newAudio.error.code : 'unknown',
            message: newAudio.error ? newAudio.error.message : 'Unknown error',
            networkState: newAudio.networkState,
            readyState: newAudio.readyState
          });
          
          reject(new Error(newAudio.error ? newAudio.error.message : 'Không thể tải file audio'));
        };
        
        // Add timeout handler to detect stuck loading
        const onStalled = () => {
          console.warn('Audio loading stalled');
        };
        
        // Add suspended handler 
        const onSuspend = () => {
        ...
        newAudio.addEventListener('stalled', onStalled);
        newAudio.addEventListener('suspend', onSuspend);
        ...
        return () => {
          ...
          newAudio.removeEventListener('stalled', onStalled);
          newAudio.removeEventListener('suspend', onSuspend);
        };
      });
      ...
      newAudio.crossOrigin = "anonymous"; // Enable CORS for the audio element
      newAudio.preload = "auto"; // Ensure preloading
      newAudio.src = cacheBustUrl;
      console.log('Setting audio source and loading:', cacheBustUrl);
      
      // Force load
      newAudio.load();
      
      // Wait for audio to be ready or timeout after 20 seconds
      await Promise.race([
        loadPromise,
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Tải audio quá thời gian')), 20000)
        )
      ]);
      ...
      const errorMsg = error.message || 'Không thể tải file audio';
      setErrorMessage(errorMsg);
      toast.error(`Lỗi tải audio: ${errorMsg}. Đang thử lại...`);
      
      // Retry loading after 3 seconds, with a maximum of 5 attempts
      if (audioRetryCountRef.current >= 5) {
        console.warn('Maximum audio load retry attempts reached');
        toast.error('Không thể tải audio sau nhiều lần thử. Vui lòng tải lại trang.');
        audioRetryCountRef.current = 0;
        return false;
      }
      
      audioRetryCountRef.current += 1;
      console.log(`Retrying audio load (attempt ${audioRetryCountRef.current}/5) in 3 seconds...`);
      
      // Display retry message to user
      toast.info(`Đang thử lại lần ${audioRetryCountRef.current}/5...`, {
        duration: 2000
      });
  ...
  */
  ...
  const calculateAdjustedScore = (score: number) => {
    ...
    let category = studentCategory.category || 'average';
    
    // Map 'weak' to 'poor' for multiplier lookup
    if (category === 'weak') category = 'poor';
  ...
  function getFillPercentage(studentCategory: string) {
