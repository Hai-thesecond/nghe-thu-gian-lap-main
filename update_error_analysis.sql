-- Update error analysis function to use our custom word similarity function
...
BEGIN
  ...
  FOR i IN 1..array_length(p_student_answers, 1) LOOP
    ...
    IF normalized_user_answer = '' THEN
      ...
      error_counts := jsonb_set(error_counts, ARRAY['blank'], to_jsonb(count_value));
      error_types := jsonb_set(error_types, ARRAY[i::TEXT], '"blank"');
    ...
    IF dictation_text_similarity(normalized_user_answer, normalized_correct_answer) <= 2 THEN
      ...
      error_counts := jsonb_set(error_counts, ARRAY['spelling'], to_jsonb(count_value));
      error_type := 'spelling';
    ...
    ELSIF (
      ...
      error_counts := jsonb_set(error_counts, ARRAY['grammar'], to_jsonb(count_value));
      error_type := 'grammar';
    ...
    ELSIF (
      ...
      error_counts := jsonb_set(error_counts, ARRAY['tense'], to_jsonb(count_value));
      error_type := 'tense';
    ...
    ELSIF array_length(string_to_array(normalized_user_answer, ' '), 1) < array_length(string_to_array(normalized_correct_answer, ' '), 1) - 1 THEN
      ...
      error_counts := jsonb_set(error_counts, ARRAY['missing'], to_jsonb(count_value));
      error_type := 'missing';
    -- Other errors (meaning)
    ELSE
      count_value := (error_counts->>'meaning')::INTEGER + 1;
      error_counts := jsonb_set(error_counts, ARRAY['meaning'], to_jsonb(count_value));
      error_type := 'meaning';
    END IF;
    
    -- Save error type and wrong answer details
    error_types := jsonb_set(error_types, ARRAY[i::TEXT], to_jsonb(error_type));
    wrong_answers := wrong_answers || jsonb_build_object(
      'index', i,
